...
csvfile = open('_data/projects.csv', 'r')
...
for row_index, row in enumerate(datareader):
	...
	# Otherwise, create a YAML file from the data in this row...
		...
		filename = 'projects/' + row[0].lower().replace(" ", "-") + '.md'
		new_yaml = open(filename, 'w')

		# Empty string that we will fill with YAML formatted text based on data extracted from our CSV.
		yaml_text = ""
		yaml_text += "---\n"
		yaml_text += "layout: project \n"
		...
		for cell_index, cell in enumerate(row):
			...
			cell_heading = data_headings[cell_index].lower().replace(" ", "-").replace("%", "percent").replace("$", "").replace(",", "")
